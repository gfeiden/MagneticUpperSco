...
\begin{table}[t]
    ...
    \begin{tabular*}{0.6\linewidth}{@{\extracolsep{\fill}}c c c}
        \hline\hline\noalign{\smallskip}
        Mass (\msun) & \teff (K) & $\langle Bf \rangle_{\rm eq,\,surf}$ (kG) \\
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.85\linewidth]{USCO5_MR.eps}
    \caption{Mass-radius diagram for stars in eclipsing systems (black points). Model predictions for magnetic and non-magnetic isochrones are the same as in Fig.~\ref{fig:ages}.}
    \label{fig:mrd}
\end{figure}
...
Introducing a magnetic perturbation as described in Sect.~\ref{sec:models}, a 10 Myr isochrone is computed and overlaid in the HR and mass-radius diagrams in Figs.~\ref{fig:ages} and \ref{fig:mrd} (thick yellow solid line). An age of 10 Myr was chosen as a starting point based on the approximate median age of the A-, F-, and G-type stars. Figures~\ref{fig:ages}a and \ref{fig:ages}c (HR diagrams) {\bf show that models}, at an age of 10 Myr with equipartition magnetic fields, are shifted toward cooler temperatures and slightly larger luminosities compared to a non-magnetic 10 Myr isochrone. Inhibition of convection by magnetic fields {\bf cools the stellar surface temperature thereby slowing} the contraction rate of young stars, which leads to stars having a larger radius and a larger luminosity at a given age. The combination of cooler surface temperatures and higher luminosities leads to a situation where a 10 Myr magnetic isochrone looks nearly identical to a non-magnetic isochrone at 5 Myr. In fact, the magnetic stellar model isochrone {\bf lies on top of} the 5 Myr standard model isochrone and {\bf within} the empirical stellar locus. Thus, a 10 Myr magnetic isochrone naturally explains the factor of two age difference observed between high- and low-mass stars, provided the high mass stars are not appreciably affected by non-standard physics (e.g., magnetic fields or rotation).
...
\section{Discussion}
\label{sec:disc}
An age of {\bf 9 -- 10} Myr gives broad agreement across the HR and mass-radius diagrams. No stellar population in Upper Scorpius {\bf studied here robustly} supports the possibility that the median age of the association is 5 Myr old.\footnote{Ages for B-type members of Upper Scorpius suggest a range of ages from 2 -- 15 Myr depending on whether models account for rotation \citep{Pecaut2012}, providing some support for a younger age.} As noted above, F-type stars appear a few million years older than stars of other spectral types. This is concerning and should be explored further, especially considering that potential missing physics in the models would likely conspire to make F-stars older, not younger. Curiously, most F stars in Upper Scorpius are at a stage of evolution where radii increase and subsequently decrease over a period of roughly 4 Myr, eerily similar to the observed age discrepancy (see Fig.\ \ref{fig:bump} and text below for more details). 

%While ages for the F stars remain in doubt, an overall age of 10 Myr provides good agreement with median age for B-type stars near the main-sequence turn-off \citep{Pecaut2012}. 
...
{\bf This observed transition is reproduced by the magnetic isochrone at 10 Myr, to some degree. For \teff\ $< 5\,000$~K, the magnetic isochrone closely matches predictions from a 5 Myr standard model isochrone, indicating magnetic inhibition of convection may be playing a role for stars at these temperatures. Compare this to the small segment of the magnetic isochrone above \teff\ $\sim 6\,000$~K, where it is beginning to converge with the 10 Myr standard model isochrone. Although the transition appears to occur over approximately the correct effective temperature domain, Fig.~\ref{fig:ages}a shows a sharp transition from a magnetic to a non-magnetic sequence between $5\,000 <$ \teff/K $< 6\,000$ whereas models predict a more gradual transition. In magnetic model space, this temperature domain corresponds to masses between $1.40$ and $1.56$ \msun. Inspection of Fig.~\ref{fig:mrd} reveals this mass range corresponds precisely to where the rapid thinning of the convection zone occurs, as expected.}
